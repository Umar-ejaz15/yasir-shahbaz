      ...
      <div className="max-w-[1240px] mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {skillsData.map((skill, index) => (
            <div
              key={index}
              className="bg-zinc-950 hover:bg-zinc-900 backdrop-blur-lg rounded-xl p-6 hover:transform hover:scale-105 transition-all duration-300"
            >
              <div className="flex items-center justify-center mb-4">
                <skill.icon className={`text-4xl ${colorClasses[skill.color]}`} />
              </div>
              <h3 className="text-xl font-bold text-center text-white mb-4">{skill.title}</h3>
              <div className="text-gray-300 space-y-2">
                {skill.skills.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    <div className={`w-20 ${colorClasses[skill.color]} rounded px-2 py-1`}>
                      {item.proficiency}
                    </div>
                    <span>{item.name}</span>
                  </div>
